# 放置?/openclaw/config/strategies/
strategy:
  name: "V5.2_5min_TrendForce"
  version: "5.2.0"
  description: "5分钟趋势确认+强制满仓+动止?
  ...
  stop_loss:
    ...
    rsi_drop: 15              # 1小时内RSI?5?
    volume_spike: 1.5         # 放量下跌


# 参代?

# 放置?/openclaw/strategies/
import pandas as pd
import numpy as np
        ...
        if self.current_layers < target_layers:
            ...
            if rsi1 < rsi_threshold:
                ...
                signals.append(Signal(
                    ...
                    layers=buy_layers,
                    price=current_price,
                    reason=f"趋势{trend_score}，RSI{rsi1:.1f}，建{buy_layers}?,
            ...
            for i, (tp_rsi, tp_layer) in enumerate(zip(tp_levels, tp_layers)):
                if rsi1 > tp_rsi and self.current_layers > 0:
                    ...
                    signals.append(Signal(
                        ...
                        layers=sell,
                        price=current_price,
                        reason=f"RSI{rsi1:.1f}>{tp_rsi}，止盈{sell}?{i+1}/3)",
                        urgency="normal"
                    ))
                    break  # 只触发一?
...
📊 操作：{'买入' if signal.side == OrderSide.BUY else '卖出'} {signal.layers}?
💰 价格?{signal.price:,.2f}
📈 原因：{signal.reason}
?紧度：{signal.urgency}
🎯 当前持仓：{self.current_layers}/5?
📍 网格中心：{self.grid_center:.0f if self.grid_center else '动?}
...
📈 趋势分：{trend_score}/5 {'🔥强趋? if trend_score>=4 else '⚡中? if trend_score>=2 else '💤观望'}
🎯 目标/当前：{target_layers}/{self.current_layers}?
📉 RSI：{rsi_str}
📊 MACD：{metrics['macd']:.2f}/{metrics['signal']:.2f}
💹 量比：{metrics.get('volume_ratio', 1):.2f}

{'?强制满仓已激? if trend_score>=4 and not self.dynamic_grid else '🔧 动网格运行中' if self.dynamic_grid else '⏸️ 待机'}
        """
        await self.tg_bot.send_message(msg, parse_mode='Markdown')
