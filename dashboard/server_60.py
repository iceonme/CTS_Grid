...
class DashboardServer60:
    """
    Dashboard 服务器 (V6.0 专用)

    功能：
    1. 专为 6.0 MTF 策略设计的独立服务
    2. 默认端口 5060
    3. 支持显示主周期(5m)和辅周期(15m)指标
    """
    ...
    def __init__(self, host='0.0.0.0', port=5066):
        self.host = host
        self.port = port
        ...
        self.app.config['SECRET_KEY'] = 'cts1-v60-secret-key'
    ...
    def _setup_routes(self):
        ...
        self.version = "v6.0-MTF-Dashboard"
        ...
        def api_status():
            ...
            if strategy_id:
                data = self._data.get(strategy_id, DashboardServer60._EMPTY_STRATEGY_DATA())
    ...
    def _setup_socketio(self):
        ...
        def handle_join(data):
            ...
            existing = self._data.get(strategy_id, DashboardServer60._EMPTY_STRATEGY_DATA())
    ...
    def register_strategy(self, strategy_id: str, display_name: str = None):
        if strategy_id not in self._data:
            self._data[strategy_id] = DashboardServer60._EMPTY_STRATEGY_DATA()
    ...
    def update(self, data: Dict[str, Any], strategy_id: str = 'default'):
        try:
            ...
            print(f'[Dashboard60] [{strategy_id}] 更新失败: {e}')

    def start(self, debug=False):
        print(f"\n{'='*60}")
        print(f"Dashboard 启动 (V6.0 版 {self.version})")
...
def create_dashboard_60(host='0.0.0.0', port=5066) -> DashboardServer60:
    return DashboardServer60(host=host, port=port)
