...
# OKX 参数
min_order_usdt = 100  # 鏈€小订单金棰?
tick_sz = 0.01       # 价格精度 (2位小鏁?
lot_sz = 0.00001     # 数量精度 (5位小鏁?

# 计算订单参数 (模拟修复后的逻辑)
test_usdt = max(100, min_order_usdt)  # 至少 100 USDT
order_btc = test_usdt / current_price
order_btc = round(order_btc, 5)       # 截断鍒?5 位小鏁?
order_px = round(current_price, 2)    # 截断鍒?2 位小鏁?
actual_cost = order_btc * order_px

print(f"\n   测试参数:")
print(f"   - 下单金额: {test_usdt} USDT")
print(f"   - 当前价格: {current_price:.2f} USDT")
print(f"   - 调整后价鏍? {order_px:.2f} USDT")
print(f"   - BTC 数量: {order_btc:.5f} BTC")
print(f"   - 实际成本: {actual_cost:.2f} USDT")

# 妫€查最小数閲?
if order_btc < lot_sz:
    print(f"\n   鉁?BTC 数量太小: {order_btc:.5f} < {lot_sz}")
    exit(1)

# 妫€查资閲?
if actual_cost > avail_bal:
    print(f"\n   鉁?资金不足: 闇€瑕?{actual_cost:.2f}, 可用 {avail_bal:.2f}")
    exit(1)
