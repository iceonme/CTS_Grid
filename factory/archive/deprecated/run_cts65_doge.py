...
from cartridges.strategies import GridStrategyV65B
...
from infra.config.api_config import OKX_DEMO_CONFIG, DEFAULT_TIMEFRAME
...
V65B_RUNTIME_PATH = "config/grid_v65b_doge_runtime.json"
V65B_FACTORY_PATH = "config/grid_v65b_doge_default.json"

STRATEGY_CATALOG = {
    'grid_v65_doge': {
        'display_name': 'Grid MTF V6.5B (DOGE-PRO)',
        'cls': GridStrategyV65B,
        'params': {
            'symbol': DOGE_SYMBOL,
            'config_path': V65B_RUNTIME_PATH,
        }
    }
}
...
def build_history_data(strategy_cls, strategy_params, initial_balance, trades_sorted, data_source):
    ...
    print(f"[V6.5B] 重建指标历史 ({len(data_source)} bars)...")
...
def main():
    print("\n" + "="*60)
    print("CTS 6.0 — 策略运行环境 (V6.5B DOGE-PRO 专用)")
    print("="*60)
    
    # 1. 配置检查
    config_path = Path(V65B_RUNTIME_PATH)
    if not config_path.exists():
        factory_path = Path(V65B_FACTORY_PATH)
        if factory_path.exists():
            import shutil
            shutil.copy(factory_path, config_path)
            print(f"[系统] 已初始化运行配置: {config_path}")

    # 2. 启动 Dashboard (Port: 50652)
    dashboard = create_dashboard_65(port=50652)
    ...
    for slot_id, cfg in STRATEGY_CATALOG.items():
        ...
        slot = StrategySlot(
            ...
            state_file=f"trading_state_{slot_id}.json",
            trades_file=f"trading_trades_{slot_id}.json"
        )
    ...
    if warmup_engine.warmup():
        ...
        hc, hrsi, heq, hmacd = build_history_data(GridStrategyV65B, STRATEGY_CATALOG['grid_v65_doge']['params'], INITIAL_BALANCE, trades_sorted, data_source)
        runner.push_warmup(first_slot, hc, hrsi, heq, hmacd)
        print(f"[V6.5B] 预热完成: {len(hc)} 根历史数据")

    # 6. 控制回调
    def on_control(action: str, str_id: str, **kwargs):
        print(f"[V6.5B Control] 接收控制指令: action={action}, str_id={str_id}, data={kwargs.get('data')}")
        try:
            if action == 'save_params':
                new_params = kwargs.get('data')
                slot = runner._slots.get(str_id)
                if slot and new_params:
                    cp = getattr(slot.strategy, 'params_path', V65B_RUNTIME_PATH)
                    ...
                    print(f"[V6.5B] 参数已热加载: {str_id}")
            elif action in ['start', 'pause', 'reset']:
                func = getattr(runner, action, None)
                if func:
                    func(str_id)
                    print(f"[V6.5B] 指令执行成功: {action} ({str_id})")
                else:
                    print(f"[V6.5B Error] MultiStrategyRunner 缺少方法: {action}")
        except Exception as e:
            print(f"[V6.5B Error] 控制回调执行失败: {e}")
            import traceback; traceback.print_exc()

    dashboard.on_control_callback = on_control

    print("\n[V6.5B] 顺利启动! 请访问 http://localhost:50652")
    ...
    while True:
        try:
            ...
            print(f"\n[V6.5B Error] 数据流中断或处理异常: {e}")
            import traceback
            traceback.print_exc()
            print("[V6.5B] 5秒后尝试重启数据流...")
            time.sleep(5)
