...
class PaperExecutor(BaseExecutor):
    ...
    def submit_order(self, order: Order) -> str:
        ...
        # 统一成交数量语义：
        # - BUY + size_in_quote=True: order.size 为报价币种金额（如 USDT），需换算为基础币数
        ...
        # 发送成交回报
        realized_pnl_val = realized_pnl if order.side == Side.SELL else None
        fill = FillEvent(
            order_id=order_id,
            symbol=order.symbol,
            side=order.side,
