...
from cartridges.strategies.grid_v70_razor import GridStrategyV70Razor
from cartridges.bridge.executors.paper import PaperExecutor
from cartridges.bridge.datafeeds import OKXDataFeed
from dashboard_70.server_70 import create_dashboard_70
...
V70_RUNTIME_PATH = "config/grid_v70_razor_btc_runtime.json"
V70_FACTORY_PATH = "config/grid_v70_razor_btc_default.json"

STRATEGY_CATALOG = {
    'grid_v70': {
        'display_name': 'Grid Razor V7.0 (纯RSI动态左侧)',
        'cls': GridStrategyV70Razor,
        'params': {
            'symbol': DEFAULT_SYMBOL,
            'config_path': V70_RUNTIME_PATH,
        }
    }
}

def build_history_data(strategy_cls, strategy_params, initial_balance, trades_sorted, data_1m, data_5m):
    """重建 V7.0-Razor 数据历史"""
    history_candles = []
    history_rsi = []
    history_equity = []
    ...
    # 手动推进状态
    temp_strat._data_1m.extend(data_1m)

    sim_cash = initial_balance
    sim_pos = 0.0
    trade_idx = 0

    print(f"[V7.0-Razor] 重建指标历史 ({len(data_5m)} bars)...")
    
    for i, data in enumerate(data_5m):
...
def main():
    print("\n" + "="*60)
    print("CTS 7.0 — 策略运行环境 (V7.0-Razor 动态网格)")
    print("="*60)
    
    # 1. 配置检查
    config_path = Path(V70_RUNTIME_PATH)
    if not config_path.exists():
        factory_path = Path(V70_FACTORY_PATH)
    ...
    # 2. 启动 Dashboard (Port: 5070)
    dashboard = create_dashboard_70(port=5070)
    ...
    for slot_id, cfg in STRATEGY_CATALOG.items():
        ...
        slot = StrategySlot(
            ...
            state_file=f"trading_state_{slot_id}_v70.json",
            trades_file=f"trading_trades_{slot_id}_v70.json"
        )
        runner.add_slot(slot)

    # 4. 数据流 (V7.0 同时需要 1m 供黑天鹅 ATR 判定，主循环使用 1m 能更好捕捉极流，由 _update_data 自行整流为 5m)
    ...
    # 5. 预热 (360 bars for 6h lookback ATR)
    print("[V7.0-Razor] 预热数据中...")
    ...
    if warmup_engine.warmup():
        data_1m = list(first_slot.strategy._data_1m)
        data_5m = list(first_slot.strategy._data_5m)
        trades_sorted = sorted(runner._trades.get(first_slot.slot_id, []), key=lambda x: str(x.get('time', '')))
        hc, hrsi, heq, hmacd = build_history_data(GridStrategyV70Razor, STRATEGY_CATALOG['grid_v70']['params'], INITIAL_BALANCE, trades_sorted, data_1m, data_5m)
        runner.push_warmup(first_slot, hc, hrsi, heq, hmacd)
        print(f"[V7.0-Razor] 预热完成: {len(hc)} 根历史数据")

    # 6. 控制回调
    def on_control(action: str, str_id: str, **kwargs):
        print(f"[V7.0 Control] 接收控制指令: action={action}, str_id={str_id}, data={kwargs.get('data')}")
        try:
            if action == 'save_params':
                new_params = kwargs.get('data')
                slot = runner._slots.get(str_id)
                if slot and new_params:
                    cp = getattr(slot.strategy, 'params_path', V70_RUNTIME_PATH)
                    ...
                    print(f"[V7.0-Razor] 参数已热加载: {str_id}")
            elif action in ['start', 'pause', 'reset']:
                func = getattr(runner, action, None)
                if func:
                    func(str_id)
                    print(f"[V7.0-Razor] 指令执行成功: {action} ({str_id})")
                else:
                    print(f"[V7.0 Error] MultiStrategyRunner 缺少方法: {action}")
        except Exception as e:
            print(f"[V7.0 Error] 控制回调执行失败: {e}")
            import traceback; traceback.print_exc()

    dashboard.on_control_callback = on_control

    print("\n[V7.0-Razor] 顺利启动! 请访问 http://localhost:5070")
