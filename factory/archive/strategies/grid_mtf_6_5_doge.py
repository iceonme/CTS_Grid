...
class GridStrategyV65B(BaseStrategy):
    """
    V6.5B 鍔ㄦ€佺綉格交易策鐣?(DOGE-PRO)
    
    基于 V6.5A 架构，针瀵?DOGE 等高波动币种优化锛?
    - 放宽 RSI 闃堝€硷紝提高参与搴?
    - 缩短冷却时间，捕捉快速机浼?
    - 减少鏈€大持仓层数，控制极端风险
    """
    ...
    def __init__(self, name: str = "Grid_V65B_DOGE", **params):
        ...
        # 数据缓存
        self._data_5m = deque(maxlen=400)   # 5m K线缓瀛?(绾?3小时，确保能看到完整日结鏋?
        self._data_15m = deque(maxlen=200)  # 15m 重采样缓瀛?
    ...
    def _load_params(self):
        ...
        # 2. 加载元数鎹?(用于 Dashboard 说明面板)
        if os.path.exists(self.meta_path):
            try:
                with open(self.meta_path, 'r', encoding='utf-8') as f:
                    self.param_metadata = json.load(f)
            except Exception as e:
                print(f"[V6.5B] 加载元数据失璐? {e}")

    def initialize(self):
        super().initialize()
        print(f"[V6.5B] {self.name} 初始化完鎴?)
    ...
    def _update_data(self, data: MarketData):
        ...
        关键：OKX 数据流每2秒推送一次同涓€鏍?分钟K线的鏈€新状态，
        必须将同涓€5分钟周期的多次推送合并为涓€条记¼，
        否则 _data_5m 里存的是2秒快鐓ц€岄潪5分钟K线，鎵€有ָ标计算都会错璇€?
        """
        ts = data.timestamp
        # 鎸?分钟取整作为当前K线的标识时间
    ...
    def _generate_signals(self, data: MarketData, context: StrategyContext) -> List[Signal]:
        ...
        if pos_size > 0 and not cooldown_lock:
            ...
            if sell_layers > 0:
                ...
                reason = f"MTF Sell [{sig_type}]: RSI={self.state.current_rsi:.1f} 抛售层数={sell_layers} 剩余持仓~={max(0, current_layers-sell_layers)}灞?
        ...
        if can_buy:
            ...
            if context.cash >= buy_usdt * 0.95:  
                reason = f"MTF Buy [{sig_type}]: RSI={self.state.current_rsi:.1f} 买入层数={buy_layers} 当前已有={current_layers}灞?
                signals.append(Signal(
                    timestamp=data.timestamp,
                    symbol=self.symbol,
                    side=Side.BUY,
    ...
    def _find_pivot_points(self, df: pd.DataFrame):
        """
        寻找鏈€近的 n 个结鏋勬€ф尝段高点和低点
        核心思路：V4.0 鐨?取最杩?N 个转折点"锛岃€岄潪"鍙?N 个最极端价格"
        """
        if len(self._data_5m) < 30: return
        
        n = 3  # 3楂?浣?
        # 屢㲿确认判定窗鍙?(增大鍒?10 鏍?= 50 分钟，过滤短期噪鐐?
        window_size = 10
        ...
        # 取最杩?n 个结构转折点 (从右向左，间隔至灏?window_size 根防重叠)
