...
class Zen7Strategy(BaseStrategy):
    ...
    鍛ㄦ湡锛氶€氳繃鍐呴儴 resample_min 鎺у埗锛堥粯璁?60 鍒嗛挓 = 1H锛?
    """
    ...
    def on_data(self, data: MarketData, context: StrategyContext) -> List[Signal]:
        ...
        # Requirement: "浠锋牸鏀剁洏鏈夋晥璺岀牬甯冩灄甯︿腑杞?. This implies waiting for bar close.
        ...
        if bar_completed:
            ...
            if pos_size > 0:
                ...
                # 1. Stop Loss: MACD姝诲弶 鎴?鏀剁洏璺岀牬涓建
    ...
    def on_fill(self, fill: FillEvent):
        ...
        if fill.side == Side.BUY:
            ...
            self.state.stats['total_trades'] += 1
            self.log(f"[{fill.timestamp}] {emoji} 骞充粨 ({sig_type}) | 浠锋牸: {fill.filled_price:.2f} | 鐩堜簭: {pnl_pct:+.2f}%")
