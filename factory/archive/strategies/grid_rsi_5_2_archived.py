"""
鍔ㄦ€佺綉鏍?RSI 策略 V5.2 (Refactored) 鈥?楂樻€ц兘瑙ｈ€︽灦鏋?

核心改进 (相较 V5.1):
  1. 架构瑙ｈ€? IncrementalIndicators / RiskController / GridEngine / DualSignalMatrix
  2. 性能革命: 鍘?Pandas, O(1) 增量指标, deque 内存优化
  3. 外部JSON配置: 35 参数可被外部 Agent 热更鏂?
...
_TREND_LABELS = {
    STRONG_BULLISH: "极强牛市 ↑↑", BULLISH: "看涨趋势 鈫?,
    NEUTRAL: "涓€у亸区间 鈫?, BEARISH: "看跌趋势 鈫?,
    STRONG_BEARISH: "极强熊市 ↓↓",
}
...
class IncrementalIndicators:
    """流式增量计算: MACD / RSI / ATR，每 tick O(1)銆?""
...
class RiskController:
    ...
    def check_death_cross(self, d: MarketData, ctx: StrategyContext, s: StrategyState) -> List[Signal]:
        ...
        # 方案A：只在零轴下方的绝对空头趋势中发生死叉才清仓，零轴上方的多头回撤不立刻清浠?
        if s.macd_line >= 0: return []
        
        # 增加 RSI 保护：如果当前处于超卖区鍩?(或接近超鍗?，则不执行趋势清仓，防止“秒买秒鍗栤€?
        # 理由：网格策略在超卖区是买入持仓期，此时即便 MACD 死叉也多Ϊ低位震荡信鍙?
...
class GridEngine:
    ...
    def find_pivots(self, highs: np.ndarray, lows: np.ndarray, times: Optional[List[float]] = None) -> Tuple[list, list]:
        ...
        for i in range(end, max(w, end - lb), -1):
            # 支撑位搜绱? i 必须鏄?[i-w, min(i+w, end)] 范围内的鏈€灏忓€?
            win_start = max(0, i - w)
            win_end = min(end, i + w)
            ...
            if highs[i] >= float(np.max(highs[win_start:win_end+1])):
                ...
                if times and i < len(times): p_data['time'] = times[i]
                ph.append(p_data)
        # 1. 计算每个鍊欓€夌偣的显著度 (Prominence)
        # 显著度定义：点位价格与局部窗鍙?[i-w, i+w] 内均值的偏差
        for p in ph:
            idx = p['index']
        ...
        # 2. ͬһ波段竞争去重 (如果两个点间璺?< w，视为同涓€个波段事件，仅留鏈€显著鐨?
    ...
    def calculate(self, highs: np.ndarray, lows: np.ndarray,
        ...
        # 1. 基础边界：周期内的全灞€鏈€楂?鏈€浣?(作为保底，确保不会漏掉显著极鍊?
        ...
        if not ph or not pl:
            ...
            # 2. 结合 Pivot 点位锛氶€夊彇 Pivot 列表中的朢㼫端鍊?
            p_upper = max(p['price'] for p in ph)
            p_lower = min(p['price'] for p in pl)
            
            # 3. 综合决策：取 Pivot 鏋佸€煎拰全局鏋佸€间腑更极端的那个
...
class GridRSIStrategyV5_2(BaseStrategy):
    """鍔ㄦ€佺綉鏍?RSI 策略 V5.2 鈥?楂樻€ц兘瑙ｈ€?+ JSON 热加杞?""
    ...
    def __init__(self, symbol: str = "BTC-USDT",
        ...
        self._data_buffer: deque = deque(maxlen=buf) # 兼容鎬? 存储 MarketData 对象
    ...
    def _apply_config_file(self):
        ...
        try:
            ...
            # 如果配置中有 symbol，同步更新策略实例的 symbol 灞炴€?
            if 'symbol' in data:
    ...
    def _maybe_reload_params(self):
        ...
        if need_reload:
            ...
            self._apply_config_file()
            self._load_param_metadata() # 同步加载元数据说鏄?
            # 如果引擎相关参数变了，重建指标引鎿?
    ...
    def initialize(self):
        ...
        print(f"[V5.2] 逻辑重置 (缓冲保留:{n}鏍?")

    def _update_buffer(self, data: MarketData):
        """核心鐘舵€佹洿鏂?(O(1) 增量更新，不生成信号)"""
    ...
    def _calculate_dynamic_grid(self, df=None, **kwargs):
        """兼容性接鍙? 包装 GridEngine 鐨?calculate 方法"""
        # 如果提供浜?df，优先使鐢?df
        if df is not None:
        ...
        # 为了满足 LiveEngine 对返鍥炲€?(upper, lower, meta) 的解构需姹?
        # 注意: GridEngine.calculate 闇€瑕?rsi_sig 参与平衡，预热时浼?0
    ...
    def _pos_layers(self, ctx: StrategyContext, cp: float) -> int:
        ...
        # 修正单λ错误: 灏?BTC 数量 * 当前价格 换算鍥?USDT 再计算层鏁?
        pos_val = pos.size * cp
    ...
    def on_data(self, data: MarketData, context: StrategyContext) -> List[Signal]:
        ...
        # 妫€查预热是否完鎴?(IncrementalIndicators 鐨勯€昏緫)
        ...
        if self.risk_ctrl.check_black_swan(self._close_buf, ts, self.state):
            for sym, pos in context.positions.items():
                if sym == self.symbol and pos.size > 0:
                    signals.append(RiskController._sell(data, sym, pos.size, "黑天鹅紧急止鎹?))
            self.state.last_candle = {'open': data.open, 'high': data.high, 'low': data.low, 'close': data.close}
            return signals
    ...
    def get_status(self, context: Optional[StrategyContext] = None) -> Dict[str, Any]:
        ...
        if s.grid_lower is not None and s.grid_upper is not None and cp > 0:
            in_grid = "低于网格" if cp < s.grid_lower else ("高于网格" if cp > s.grid_upper else "网格鍐?)
