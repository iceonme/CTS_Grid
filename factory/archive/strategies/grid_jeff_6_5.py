...
class GridJeff65Strategy(BaseStrategy):
            ...
            if sell_layers > 0:
                ...
                if sell_btc > 0:
                    ...
                    if self.min_profit_filter and data.close <= avg_cost * (1 + self.min_profit_ratio):
                        # 濡傛灉寮€鍚簡鍙泩涓嶄簭锛屽苟涓斿綋鍓嶄环鏍兼瘮杩欐壒鍗栧嚭灞傜殑鎴愭湰浠疯繕浣庯紝鍒欐斁寮冩湰娆″崠鍑轰俊鍙?
                        sell_btc = 0
                        sell_layers = 0
                
                # 濡傛灉鏄渶鍚庝竴灞傦紝鍏ㄥ钩浠撻槻姝㈢矇灏?
    ...
    def on_fill(self, fill: FillEvent):
        ...
        if fill.side == Side.BUY:
            ...
            self.log(f"[{fill.timestamp}] 寮€浠?{sig_type} | 浠锋牸: {fill.filled_price:.2f} | 鏁伴噺: {fill.filled_size:.4f} | 褰撳墠灞傛暟: {len(self.state.entry_prices)}")
            ...
            self.log(f"[{fill.timestamp}] 骞充粨 {sig_type} | 浠锋牸: {fill.filled_price:.2f} | 鎴愭湰: {avg_pop:.2f} | 鐩堜簭: {pnl_pct:+.2f}% | 鏁伴噺: {fill.filled_size:.4f} | 褰撳墠灞傛暟: {len(self.state.entry_prices)}")
