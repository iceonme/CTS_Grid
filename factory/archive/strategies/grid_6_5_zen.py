...
class GridZen65Strategy(GridJeff65Strategy):
    """
    Zen 6.5 策略锛?
    1. 继承 Jeff 6.5A 逻辑 (RSI 触发, MACD 仓位娴?
    2. 固定本金: 仓位始终基于初始 10000 计算，不复利銆?
    3. 全仓止损: 当全仓盈亏低浜?-2.5% 时，清仓离场銆?
    """
    def __init__(self, name="Grid_Zen_65", **params):
        # 强制设置 min_profit_filter 涓?True，符鍚?Zen 鐨勨€滅泩利提鍙栤€濋€昏緫（必须盈利才出）
        params['min_profit_filter'] = True
        super().__init__(name, **params)
        
        self.stop_loss_threshold = params.get('stop_loss_threshold', -0.10) # 修正涓?-10%
    ...
    def on_data(self, data: MarketData, context: StrategyContext) -> List[Signal]:
        ...
        # 卖出逻辑 (优化卖点)
            ...
            # 优化加餐：动量减弱卖鐐?
                ...
                # RSI 钩头向下 涓?MACD 红柱缩短 涓?处于超买区边缂?
                sell_layers = 1
            ...
            if sell_layers > 0:
                ...
                avg_cost = cost_sum / (sell_layers * (self.layer_value / cost_basis)) if layers > 0 else 0 # 箢㻯估绠?
                # 重新精确计算
                sell_btc_est = 0
                cost_sum_exact = 0
    ...
    def on_fill(self, fill: FillEvent):
        ...
        if fill.side == Side.SELL and sig_type == 'STOP_LOSS':
            ...
            self.log(f"[{fill.timestamp}] 鉂?止损完成 | 价格: {fill.filled_price:.2f} | 盈亏: {pnl_pct:+.2f}% | 数量: {fill.filled_size:.4f}")
        else:
            super().on_fill(fill)
