...
def run_arena_viewer():
    ...
    parser.add_argument("--strategy", type=str, default="grid_v85", help="策略名称，支鎸侀€楀彿分隔 (grid_v85,grid_mtf_6_0)")
    parser.add_argument("--start", type=str, default="2025-03-16", help="寮€始日鏈?(YYYY-MM-DD)")
    ...
    print(f"[竞技场] 策略列表: {strategy_ids} | 数据婧? {data_path} | 周期: {args.start} 鍒?{args.end}")
    ...
    print(f"[性能] 已加杞?{len(df_slice)} 鏍?K 线，寮€始运行多策略并行仿真...")
    ...
    # 兼容性处理：如果只有涓€个策略，同时也放在外层以便旧鐗?UI 读取 (鍙€夛紝建议ֱ接升级 UI)
    # 这里我们直接采用新结构，并去升级 UI
