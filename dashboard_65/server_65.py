...
class DashboardServer65:
    """
    Dashboard 服务器 (V6.5 专用)

    功能：
    1. 专为 6.5 MTF 策略设计的独立服务
    2. 默认端口 5065
    ...
    def __init__(self, host='0.0.0.0', port=5065):
        self.host = host
        self.port = port
        ...
        self.app.config['SECRET_KEY'] = 'cts1-v65-secret-key'
    ...
    def _setup_routes(self):
        ...
        self.version = "v6.5-MTF-Dashboard"
        ...
        def api_status():
            ...
            if strategy_id:
                data = self._data.get(strategy_id, DashboardServer65._EMPTY_STRATEGY_DATA())
    ...
    def _setup_socketio(self):
        ...
        def handle_join(data):
            ...
            existing = self._data.get(strategy_id, DashboardServer65._EMPTY_STRATEGY_DATA())
    ...
    def register_strategy(self, strategy_id: str, display_name: str = None):
        if strategy_id not in self._data:
            self._data[strategy_id] = DashboardServer65._EMPTY_STRATEGY_DATA()
    ...
    def start(self, debug=False):
        print(f"\n{'='*60}")
        print(f"Dashboard 启动 (V6.5 版 {self.version})")
...
def create_dashboard_65(host='0.0.0.0', port=5065) -> DashboardServer65:
    return DashboardServer65(host=host, port=port)
