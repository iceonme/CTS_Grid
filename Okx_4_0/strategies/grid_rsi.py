...
class GridRSIStrategy(BaseStrategy):
    ...
    def _calculate_rsi(self, prices: pd.Series) -> float:
        ...
        # 使用指数移动平均 (Wilder's: alpha = 1/period)
                ...
                if lows[i] <= l_min:
                    ...
                    if is_pivot:
                        ...
                        if not pivot_lows or abs(lows[i] - pivot_lows[-1]['price']) > (lows[i] * 0.001):
                            ts = times[i]
                            # 转换为毫秒时间戳 (考虑到 pandas Timestamp 可能带有时区)
                            ts_ms = int(ts.timestamp() * 1000)
                            pivot_lows.append({'price': float(lows[i]), 'time': str(ts), 't': ts_ms, 'type': p_type})
                ...
                if highs[i] >= l_max:
                    ...
                    if is_pivot:
                        if not pivot_highs or abs(highs[i] - pivot_highs[-1]['price']) > (highs[i] * 0.001):
                            ts = times[i]
                            ts_ms = int(ts.timestamp() * 1000)
                            pivot_highs.append({'price': float(highs[i]), 'time': str(ts), 't': ts_ms, 'type': p_type})
                ...
                if last_low > grid_price and current_low <= grid_price:
                    ...
                    signals.append(Signal(
                        ...
                        meta={
                            ...
                            'grid_upper': self.state.grid_upper,
                            'grid_lower': self.state.grid_lower,
                            'rsi': self.state.current_rsi,
                            'level': self._estimate_position_layers(context, current_price) + 1
                        },
                    ))
                    break
                    ...
                    if current_pos and current_pos.avg_price < current_price * (1 - grid_interval_pct):
                        ...
                        signals.append(Signal(
                            ...
                            meta={
                                ...
                                'grid_lower': self.state.grid_lower,
                                'rsi': self.state.current_rsi,
                                'level': self._estimate_position_layers(context, current_price)
                            },
                        ))
                        break
