...
let currentStrategyId = window.STRATEGY_ID || 'grid-v60';
...
function renderSlotList(slots) {
    ...
    const countHeader = document.getElementById('slotCountHeader');
    if (!container || !countHeader) return;

    countHeader.textContent = `${slots.length} 项`;
    if (slots.length === 0) {
        container.innerHTML = '<div class="empty-state" style="padding: 10px 0;">暂无活跃槽位</div>';
        return;
    }

    // 按层级索引倒序排列 (后进先出感官)
    const sortedSlots = [...slots].sort((a, b) => b.idx - a.idx);
    
    container.innerHTML = sortedSlots.map(s => {
        const timeStr = s.time ? new Date(s.time).toLocaleTimeString('zh-CN', { hour12: false }) : '--:--';
        const amount = s.price * s.size;
        return `
            <div style="display: flex; justify-content: space-between; align-items: center; padding: 6px 8px; background: rgba(255,255,255,0.03); border-radius: 4px; margin-bottom: 4px; border: 1px solid rgba(255,255,255,0.02);">
                <div style="display: flex; align-items: center; gap: 8px;">
                    <div style="width: 24px; height: 18px; line-height: 18px; text-align: center; background: ${s.idx < 0 ? 'rgba(0, 208, 132, 0.15)' : 'rgba(255, 71, 87, 0.15)'}; color: ${s.idx < 0 ? '#00d084' : '#ff4757'}; border-radius: 3px; font-weight: bold; font-family: monospace;">L${s.idx}</div>
                    <div style="display: flex; flex-direction: column;">
                        <div style="color: var(--text-primary); font-family: monospace; font-size: 13px;">$${s.price.toFixed(1)}</div>
                        <div style="color: var(--text-secondary); font-size: 10px; opacity: 0.7;">成本价</div>
                    </div>
                </div>
                <div style="text-align: right;">
                    <div style="color: var(--primary); font-weight: bold; font-size: 13px;">$${amount.toFixed(2)}</div>
                    <div style="display: flex; flex-direction: column; align-items: flex-end;">
                        <span style="color: var(--text-secondary); font-size: 10px;">${s.size.toFixed(4)} BTC</span>
                        <span style="color: var(--text-secondary); font-size: 9px; opacity: 0.5;">${timeStr}</span>
                    </div>
                </div>
            </div>
        `;
