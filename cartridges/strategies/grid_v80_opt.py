...
class GridStrategyV80Opt(BaseStrategy):
    ...
    def __init__(self, name: str = "Grid_V80_OPT", **params):
        ...
        class StrategyState:
            ...
            # --- 6h 计算出的核心网格灞炴€?---
            volatility: float = 0.0         # 6h 测算出的震幅波动鐜?
            base_top: float = 0.0           # 6h 5鍙? 顶部中枢
            base_bottom: float = 0.0        # 6h 5鍙? 底部中枢
            active_layers_mode: int = 5     # 当前实际婵€活的层级模式 (5 鎴?7)
            ...
            # 严格防复吸占用字鍏? key = c_idx, value = True 表ʾ本层已建仓且未平浠?
    ...
    def _load_params(self):
        ...
        if os.path.exists(self.meta_path):
            try:
                ...
                print(f"[V8.0-OPT] 加载元数据失璐? {e}")

    def initialize(self):
        super().initialize()
        print(f"[V8.0-OPT] {self.name} 初始化完鎴?)
    ...
    def _update_data(self, data: MarketData):
        """(兼容性保留，涓婚€昏緫已改鐢?on_data 直接 append)"""
        pass
    ...
    def _rebalance_grid_if_needed(self, data: MarketData):
        """执行网格核心算法与参数自适应 - 支持6Сʱ常规模式鎴?小时熔断重置模式"""
    ...
    def _calculate_4h_grid_with_preserve(self, data: MarketData):
        """计算4小时网格（熔断超ʱ后的重置模ʽ）"""
        print(f"[V8.0-OPT] 寮€始计绠?小时网格（熔断重置模式）")
        
        # 使用4小时数据
        lookback_bars = (4 * 60) // self._timeframe_mins  # 4小时 = 240鏍?分钟绾?
        if len(self._data_main) < lookback_bars:
            print(f"[V8.0-OPT] 4小时数据不足({len(self._data_main)}/{lookback_bars})，使用全部可用数鎹?)
            lookback_bars = len(self._data_main)
        
        # 执行4小时网格计算
        self._calculate_grid_core(data, lookback_bars, hours_label="4h")
        
        print(f"[V8.0-OPT] 4小时网格重置完成 | 保留全部持仓作为新网格底浠?)
    ...
    def _calculate_grid_core(self, data: MarketData, lookback_bars: int, hours_label: str = "6h"):
        ...
        if volatility > vol_threshold:
            active_layers = base_layers + 2  # 7层模寮?
        else:
            active_layers = base_layers      # 5层模寮?
        ...
        print(f"[V8.0-OPT] 重算网格完成 [{hours_label}] | Vol={volatility*100:.2f}% ({active_layers}灞? RSI=[{self.state.dynamic_rsi_buy}-{self.state.dynamic_rsi_sell}]")

    def _check_risk_and_halt(self, data: MarketData, context: Optional[StrategyContext]) -> bool:
        """
        完整风控妫€查：双向熔断(智能观察模式) + 黑天楣?+ 连续熔断封印
        返回 True = 阻断交易
        """
        now = data.timestamp
        ...
        # 1.2 黑天鹅模式（减仓保护锛?
        if self.state.black_swan_mode:
            return True  # 阻断新开仓，鐢?_process_black_swan_exit 处理减仓
    ...
    def _enter_observation_mode(self, now: datetime, direction: str, trigger_price: float):
        ...
        print(f"[V8.0-OPT] 熔断触发 | 方向: {direction} | 触发浠? {trigger_price}")
        print(f"[V8.0-OPT] 进入2小时观察鏈?| 持仓不变 | 等待价格回归网格...")

    def _handle_observation_mode(self, data: MarketData, now: datetime) -> bool:
        """处理观察鏈熼€昏緫 - 返回True表示继续阻断，False表ʾ恢复交易"""
        ...
        # 情况2: 婊?Сʱ未回褰?鈫?重置4小时网格
        cooldown_seconds = 2 * 3600  # 2小时 = 7200绉?
        if elapsed >= cooldown_seconds:
            self._reset_grid_after_timeout(now)
            return False  # 新网格已生成，恢复交鏄?
        
        # 情况3: 仍在观察涓?鈫?阻断交易，每5分钟打印日志
        if int(elapsed) % 300 == 0 and elapsed > 0:
            remaining = cooldown_seconds - elapsed
            print(f"[V8.0-OPT] 观察涓?| 已过: {elapsed/60:.0f}鍒?| 剩余: {remaining/60:.0f}鍒?| 价格: {current_price:.2f}")
    ...
    def _reset_grid_after_timeout(self, now: datetime):
        ...
        print(f"[V8.0-OPT] 观察期满2小时 | 价格未回褰?| 启动4小时网格重置")
        ...
        # 标记闇€要重置网格（使用4小时数据），鍦?_rebalance_grid_if_needed 中处鐞?
        self.state.last_rebalance_time = None
        self.state.use_4h_grid = True  # 标记使用4小时模式
        
        print(f"[V8.0-OPT] 新网格参数已设置 | 下次重平衡将使用4小时数据并保留持浠?)
    ...
    def _check_black_swan_trigger(self, data: MarketData, bs_cfg: dict) -> bool:
        ...
        if self.state.current_atr >= self.state.atr_ma * bs_cfg.get('atr_multiplier', 3.0):
            ...
            print(f"[V8.0-OPT] 触发黑天鹅熔鏂?| ATR: {self.state.current_atr:.2f} | 均线: {self.state.atr_ma:.2f}")
            print(f"[V8.0-OPT] 停止新开仓，启动姣?0分钟自动市价减仓程序")
            return True
    ...
    def _check_consecutive_limit(self, now: datetime, cb_cfg: dict) -> bool:
        ...
        if len(self._circuit_breaker_history) >= limit:
            ...
            print(f"[V8.0-OPT] 连续熔断保护 | 24小时内已熔断{len(self._circuit_breaker_history)}娆?)
            print(f"[V8.0-OPT] 进入12小时封印鏈?| 完ȫ停止交易")
            return True
    ...
    def _resume_from_meltdown(self):
        ...
        self.state.last_rebalance_time = None
        print(f"[V8.0-OPT] 封印解除 | 恢复交易，强制刷新基鍑?)
    ...
    def _process_black_swan_exit(self, data: MarketData, context: StrategyContext) -> List[Signal]:
        ...
        if pos_size <= 0:
            ...
            self.state.is_halted = False
            self.state.halt_reason = ""
            print(f"[V8.0-OPT] 黑天鹅已被化解，已完成全仓清鐩樸€傜瓑待重鍚€?)
            return []

        # 若价格反弹超成本浠?鈫?停止减仓，恢复正甯?
        if data.close >= float(pos.avg_price):
            print(f"[V8.0-OPT] 黑天鹅期间强劲反弹，价格已覆盖平均成本，撤销警报銆?)
            self.state.black_swan_mode = False
            self.state.is_halted = False
            self.state.halt_reason = ""
            return []
    ...
    def _generate_signals(self, data: MarketData, context: StrategyContext) -> List[Signal]:
        ...
        # 严格防复吸隔离：当前区间层若被建仓过，无论距离多Զ都不再追高或补浠?
    ...
    def get_status(self, context: Optional[StrategyContext] = None) -> Dict[str, Any]:
        ...
        if self.state.is_halted:
            if self.state.black_swan_mode:
                signal_text = f"黑天鹅熔鏂?(逐步减仓保护)"
            else:
                signal_text = f"熔断冷却涓?({self.state.halt_reason})"
            signal_color = "sell"
        ...
        return {
            ...
            'volatility_state': f"{self.state.volatility*100:.2f}% ({self.state.active_layers_mode}层实浣?",
            'marketRegime': f"模式: {self.state.active_layers_mode}层实浣?,
            'vol_trend': '黑天鹅防御开鍚? if self.state.black_swan_mode else '鑷€傚簲触发',
            ...
            'grid_range': f"{lower_bound:.1f} - {upper_bound:.1f}" if lower_bound > 0 else "计算涓?..",
            'grid_lines': gl,
