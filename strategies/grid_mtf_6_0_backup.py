...
class GridMTFStrategyV6_0(BaseStrategy):
    ...
    def _rebuild_grid(self):
        ...
        if new_grid:
            ...
            if layers:
                for l in layers:
                    lines.append(l["bottom"])
                lines.append(layers[-1]["top"])
                    ...
                    if self.state.last_sell_price > 0 and abs(price - self.state.last_sell_price) / self.state.last_sell_price < price_buff:
                        ...
                        if sell_size > 0:
                            ...
                            signals.append(Signal(
                                ...
                                size=sell_size, reason=f"LIFO Sell (Idx:{t_idx})",
                                meta={
                                    "rsi": rsi,
                                    "layer_idx": t_idx,
        ...
        if layer_idx < 0:
            ...
            if price <= current_layer["mid"] and not current_layer["locked"]:
                if current_layer["type"] == "VIRTUAL":
                    ...
                    if rsi < rsi_buy_thr: is_triggered = True
                else:
                    is_triggered = True
            ...
            if is_triggered:
                ...
                if context.cash >= buy_usdt * 0.98:
                    signals.append(Signal(
                        ...
                        reason=f"Slot Buy (Idx:{layer_idx})"
                    ))
